...
impl FuncSection {
    fn push(&mut self, typeidx: TypeIdx) -> FuncIdx {
        let funcidx = FuncIdx{ idx: self.content.len() as u32 + self.idx_offset }; // the offset is the number of imported functions, which precedes the functions defined in this module
        self.content.push(typeidx);
        funcidx
	}
    fn new_with_offset(idx_offset: u32) -> FuncSection {
        FuncSection{ idx_offset: idx_offset, ..Default::default() }
	}
}

impl TableSection {
    fn new_with_offset(idx_offset: u32) -> TableSection {
        TableSection{ idx_offset: idx_offset, ..Default::default() }
	}
}

impl MemSection {
    fn new_with_offset(idx_offset: u32) -> MemSection {
        MemSection{ idx_offset: idx_offset, ..Default::default() }
	}
}

impl GlobalSection {
    fn new_with_offset(idx_offset: u32) -> GlobalSection {
        GlobalSection{ idx_offset: idx_offset, ..Default::default() }
	}
}
