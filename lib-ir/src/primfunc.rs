...
fn generate_number_binary_operator_primitive(prim_inst: PrimInst) -> Func
{
    ...
    let mut func = Func::new_with_params_and_result(&[VarType::Number, VarType::Number], VarType::Number);
    ...
    func.statements.push(
        Statement::Return{
            expr: Expr{
                ...
                kind: ExprKind::PrimAppl{prim_inst: prim_inst, args:
                    Box::new([
                        Expr{
                            vartype: VarType::Number,
...
fn generate_number_binary_operator_wrapper(prim_funcidx: FuncIdx) -> Func
{
    ...
    func.signature_filter.push((Box::new([VarType::Number, VarType::Number]), VarType::Number, prim_funcidx));
 ...
 */
fn generate_number_binary_operator(funcs: &mut Vec<Func>, builtin_funcidxs: &mut [FuncIdx; NUM_BUILTINS as usize], prim_inst: PrimInst, builtin: Builtin)
{
    let primitive_funcidx = indexed_push(funcs, generate_number_binary_operator_primitive(prim_inst));
    let builtin_funcidx = indexed_push(funcs, generate_number_binary_operator_wrapper(primitive_funcidx));
