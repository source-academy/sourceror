...
export function destroyContext(context: Context) {
  LoadWasm().then((module) => module.destroy_context(context));
...
export function compileRepl(context: Context, code: string) {
  return LoadWasm().then((module) => module.compile_repl(context, code));
}
...
async function compilerFetchDep(
  context: Context,
  name: string
): Promise<string> {
