...
fn encode_program(ir_program: &ir::Program) -> wasmgen::WasmModule
{
    ...
    let struct_field_byte_offsets: Box<[Box<[u32]>]> = ir_program.struct_types.iter().map(|struct_type| {
        struct_type
            .iter()
    ...
    encode_funcs(&ir_program.funcs, &ir_program.struct_types, &struct_field_byte_offsets, &mut wasm_module);
...
impl<'a> Scratch<'a> {
    ...
    {
        Self::push_impl(&mut self.locals_builder, wasmgen::ValType::I32, &mut self.i32_buffer, &mut self.i32_idx)
	}
    fn push_i64(&mut self) -> wasmgen::LocalIdx
    {
        Self::push_impl(&mut self.locals_builder, wasmgen::ValType::I64, &mut self.i64_buffer, &mut self.i64_idx)
	}
    fn push_f32(&mut self) -> wasmgen::LocalIdx
    {
        Self::push_impl(&mut self.locals_builder, wasmgen::ValType::F32, &mut self.f32_buffer, &mut self.f32_idx)
	}
    fn push_f64(&mut self) -> wasmgen::LocalIdx
    {
        Self::push_impl(&mut self.locals_builder, wasmgen::ValType::F64, &mut self.f64_buffer, &mut self.f64_idx)
	}
    fn pop_i32(&mut self)
    {
    ...
    fn push_impl(locals_builder: &mut wasmgen::LocalsManager, valtype: wasmgen::ValType, buffer: &mut Vec<wasmgen::LocalIdx>, idx: &mut usize) -> wasmgen::LocalIdx
    {
        if *idx == buffer.len() {
...
fn encode_funcs(ir_funcs: &[ir::Func], ir_struct_types: &[Box<[ir::VarType]>], ir_struct_field_byte_offsets: &[Box<[u32]>], wasm_module: &mut wasmgen::WasmModule)
{
    struct WasmRegistry {
    ...
    let mut registry_list: Box<[WasmRegistry]> = ir_funcs.iter().map(|ir_func| {
        ...
        let var_map = param_map.iter().cloned().chain(local_map.into_iter().map(|x| num_wasm_params + x)).map(|x| wasmgen::LocalIdx{idx:x}).collect();
        let params_locals_types = ir_func.params.iter().chain(ir_func.locals.iter()).cloned().collect();
        WasmRegistry {
    ...
    ir_funcs.iter().enumerate().for_each(|(ir_funcidx, ir_func)| {
        ...
        encode_statements(ir_func.statements.as_slice(), ctx, &mut mutctx, expr_builder);
...
{
    let converted_params: Box<[&'static [wasmgen::ValType]]> = ir_params.iter().map(|ir_param| encode_vartype(*ir_param)).collect();
    let param_map = converted_params.iter().cloned().map(|param| param.len() as u32).collect();
    let wasm_params = converted_params.iter().flat_map(|param| param.iter()).cloned().collect();
    (wasm_params, param_map)
}

fn encode_result(ir_results: &Option<ir::VarType>) -> Box<[wasmgen::ValType]>
{
    ir_results.iter().flat_map(|ir_result| encode_vartype(*ir_result).iter()).cloned().collect()
}
...
// net wasm stack: [] -> []
fn encode_statements(statements: &[ir::Statement], ctx: EncodeContext, mutctx: &mut MutContext, expr_builder: &mut wasmgen::ExprBuilder)
{
...
// net wasm stack: [] -> []
fn encode_statement(statement: &ir::Statement, ctx: EncodeContext, mutctx: &mut MutContext, expr_builder: &mut wasmgen::ExprBuilder)
{
    match statement {
        ...
        ir::Statement::If{cond, true_stmts, false_stmts} => unimplemented!(),
...
fn encode_target_addr_pre(target: &ir::TargetExpr, incoming_vartype: ir::VarType, ctx: EncodeContext, mutctx: &mut MutContext, expr_builder: &mut wasmgen::ExprBuilder)
{
    match target {
        ...
        ir::TargetExpr::Local{localidx, next} => {
            match next {
                ...
                Some(struct_field) => {
                    ...
                    assert!(ctx.params_locals_types[*localidx] == ir::VarType::StructT{typeidx: struct_field.typeidx}, "ICE: IR->Wasm: Struct type incorrect or not proved at compilation time");
                    // net wasm stack: [] -> [struct_ptr]
                    expr_builder.local_get(ctx.var_map[*localidx]);

                    fn follow_nested_struct(outer_struct_field: &ir::StructField, ctx: EncodeContext, expr_builder: &mut wasmgen::ExprBuilder) {
                        if let Some(inner_struct_field) = &outer_struct_field.next {
                            ...
                            expr_builder.i32_load(wasmgen::MemArg::new4(ctx.struct_field_byte_offsets[outer_struct_field.typeidx][outer_struct_field.fieldidx]));
...
fn encode_target_addr_post(target: &ir::TargetExpr, incoming_vartype: ir::VarType, ctx: EncodeContext, mutctx: &mut MutContext, expr_builder: &mut wasmgen::ExprBuilder)
{
    match target {
        ir::TargetExpr::Global{globalidx, next} => {
            unimplemented!("Targetting a global is not implemented yet");
        ...
        ir::TargetExpr::Local{localidx, next} => {
            match next {
                None => {
                    encode_store_local(ctx.var_map[*localidx], ctx.params_locals_types[*localidx], incoming_vartype, expr_builder);
                },
                Some(struct_field) => {
                    ...
                    fn get_innermost_offset(sf: &ir::StructField, ctx: EncodeContext) -> (u32, ir::VarType) {
                        if let Some(inner_struct_field) = &sf.next {
                            return get_innermost_offset(inner_struct_field, ctx);
						}
                        else {
                            return (ctx.struct_field_byte_offsets[sf.typeidx][sf.fieldidx], ctx.struct_types[sf.typeidx][sf.fieldidx]);
						}
					}

                    let (offset, ir_dest_vartype) = get_innermost_offset(struct_field, ctx);
                    encode_store_memory(offset, ir_dest_vartype, incoming_vartype, &mut mutctx.scratch, expr_builder);
				}
			}
		}
	}
...
fn encode_store_memory(wasm_struct_offset: u32, ir_dest_vartype: ir::VarType, ir_source_vartype: ir::VarType, scratch: &mut Scratch, expr_builder: &mut wasmgen::ExprBuilder)
{
    if ir_dest_vartype == ir_source_vartype {
        match ir_dest_vartype {
            ir::VarType::Any => {
...
fn encode_expr(expr: &ir::Expr, ctx: EncodeContext, mutctx: &mut MutContext, expr_builder: &mut wasmgen::ExprBuilder)
{
    match &expr.kind {
        ir::ExprKind::PrimUndefined => {
            assert!(expr.vartype == ir::VarType::Undefined, "ICE: IR->Wasm: PrimUndefined does not have type undefined");
            // Don't do anything, because undefined is encoded as <nothing>
        },
        ir::ExprKind::PrimNumber{val} => {
            assert!(expr.vartype == ir::VarType::Number, "ICE: IR->Wasm: PrimNumber does not have type number");
            expr_builder.f64_const(*val);
        },
        ir::ExprKind::PrimBoolean{val} => {
            assert!(expr.vartype == ir::VarType::Boolean, "ICE: IR->Wasm: PrimBoolean does not have type boolean");
            expr_builder.i32_const(if *val { 1 } else { 0 });
        },
        ir::ExprKind::PrimString{val} => {
            assert!(expr.vartype == ir::VarType::String, "ICE: IR->Wasm: PrimString does not have type string");
            unimplemented!("PrimString needs GC support, unimplemented");
        },
        ir::ExprKind::PrimStructT{typeidx} => {
            assert!(expr.vartype == ir::VarType::StructT{typeidx: *typeidx}, "ICE: IR->Wasm: PrimStructT does not have correct type, or typeidx is incorrect");
            unimplemented!("PrimStructT needs GC support, unimplemented");
        },
        ir::ExprKind::PrimFunc{funcidx, closure} => {
            assert!(expr.vartype == ir::VarType::Func, "ICE: IR->Wasm: PrimFunc does not have type func");
        ...
        ir::ExprKind::TypeOf{expr, expected} => {
            assert!(expr.vartype == ir::VarType::Any, "ICE: IR->Wasm: LHS of TypeOf builtin should have static type Any");
...
fn encode_target_value(source: &ir::TargetExpr, outgoing_vartype: ir::VarType, ctx: EncodeContext, mutctx: &mut MutContext, expr_builder: &mut wasmgen::ExprBuilder)
{
    match source {
        ir::TargetExpr::Global{globalidx, next} => {
        ...
        ir::TargetExpr::Local{localidx, next} => {
            ...
            match next {
                None => {
                    ...
                    encode_load_local(ctx.var_map[*localidx], ctx.params_locals_types[*localidx], outgoing_vartype, expr_builder);
				},
                Some(struct_field) => {
                    // For Source1, structs can only arise from closures and local variables, so their types must already be known at compilation time.
                    assert!(ctx.params_locals_types[*localidx] == ir::VarType::StructT{typeidx: struct_field.typeidx}, "ICE: IR->Wasm: Struct type incorrect or not proved at compilation time");
                    ...
                    fn load_inner_local(outer_struct_field: &ir::StructField, outgoing_vartype: ir::VarType, ctx: EncodeContext, mutctx: &mut MutContext, expr_builder: &mut wasmgen::ExprBuilder)
                    {
                        if let Some(inner_struct_field) = &outer_struct_field.next {
                            // If there is an inner struct, it must be known at compilation time
                            assert!(ctx.struct_types[outer_struct_field.typeidx][outer_struct_field.fieldidx] == ir::VarType::StructT{typeidx: inner_struct_field.typeidx}, "ICE: IR->Wasm: Struct type incorrect or not proved at compilation time");
                            ...
                            expr_builder.i32_load(wasmgen::MemArg::new4(ctx.struct_field_byte_offsets[outer_struct_field.typeidx][outer_struct_field.fieldidx]));
                            // recurse if there are even more inner structs
                            load_inner_local(&inner_struct_field, outgoing_vartype, ctx, mutctx, expr_builder);
						}
                        else {
                            let offset: u32 = ctx.struct_field_byte_offsets[outer_struct_field.typeidx][outer_struct_field.fieldidx];
                            let ir_source_vartype: ir::VarType = ctx.struct_types[outer_struct_field.typeidx][outer_struct_field.fieldidx];
                            encode_load_memory(offset, ir_source_vartype, outgoing_vartype, &mut mutctx.scratch, expr_builder);
						}
					}
